<?php
/**
 * Template Name: Products (Capstone)
 * Product listing from products.json; AJAX add-to-cart.
 */
    ...
    <div class="products-grid">
        <?php foreach ( $products as $p ) : ?>
            ...
            if ( $img && strpos( $img, 'http' ) !== 0 && strpos( $img, '//' ) !== 0 ) {
                // Normalize path: JSON uses images/product1.jpg but files are under /image/.
            ...
            // Optional extra fields if they exist in products.json
            $category    = $p['category'] ?? '';
            $origin      = $p['origin'] ?? '';
            $supplier    = $p['supplier'] ?? '';
            ...
            <div class="product-card">
                ...
                <div class="product-actions">
                    ...
                    <button type="button"
                            ...
                            onclick="tennisproOpenProductModal(this)">
                        View details
                    </button>
                </div>
            </div>
    ...
    <script>
        (function () {
            ...
            window.tennisproOpenProductModal = function (btn) {
                ensureModal();
                if (!modal || modal.style.display === 'flex') return; // one at a time
                ...
                var baseDesc = (p.description || '') + '';
                var extraDesc = ' This package is part of the TennisPro Hub capstone store. All bookings are for demonstration only and will not charge your real card.';
                descEl.textContent = baseDesc + extraDesc;

<?php
/**
 * Snippet: TennisPro My Orders
 * Shortcode: [tennispro_my_orders]
 * Description: Customer account page – order history, saved cards, order details.
 */

add_shortcode( 'tennispro_my_orders', function () {
    if ( empty( $_SESSION['checkout_email'] ) ) {
        return '<div class="alert alert-error">Please <a href="' . esc_url( get_permalink( get_page_by_path( 'login' ) ) ?: home_url( '/login/' ) ) . '">log in</a> to view your orders.</div>';
    }
        ...
        if ( $order_id > 0 ) {
            ...
            if ( $current_order ) {
                $current_order_items = $wpdb->get_results(
                    $wpdb->prepare(
                        "SELECT product_id, quantity, unit_price FROM {$prefix}order_items WHERE order_id = %d",
                        $order_id
                    ),
                    ARRAY_A
                );
    ...
    <div class="my-orders-layout">
        ...
        <main class="my-orders-main">
            ...
            <section class="account-section">
                <h2>Saved cards</h2>
                <?php $cards = function_exists( 'tennispro_get_customer_cards' ) ? tennispro_get_customer_cards( $customer_email ) : []; ?>
                <?php if ( $cards ) : ?>
                    <div class="table-responsive">
                    <table class="admin-table">
                        <thead><tr><th>Holder</th><th>Brand</th><th>Number</th><th>Added at</th></tr></thead>
                        <tbody>
            ...
            <?php if ( $current_order && $current_order_items ) : ?>
                <section class="order-details">
                    ...
                    <p>Placed on <?php echo esc_html( $current_order['created_at'] ); ?>, total $<?php echo number_format( (float) $current_order['total_amount'], 2 ); ?>.</p>
                    <div class="table-responsive">
                    <table class="admin-table">
